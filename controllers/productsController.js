...
exports.getSellersProducts = (req, res, next) => {
  try {
    ...
    database.query(query, [id], (err, result) => {
      if (err) throw err;
      const getImage = "SELECT * FROM all_images WHERE product_id=?";
      database.query(getImage, [result[0].product_id], (err, resultImg) => {
        if (err) throw err;
        res.status(200).json({ data: result, image: resultImg });
      });
    });
...
exports.deleteProduct = async (req, res, next) => {
  const { id } = req.params;
  try {
    const result = await cloudinary.v2.uploader.destroy(image.id);
    ...
    database.query(deleteQuery, (err, result) => {
      if (err) {
        ...
        database.query(deleteImgQuery, (err, result) => {
